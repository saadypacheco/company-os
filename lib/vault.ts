...
export const DEFAULT_VAULT: VaultProject[] = [
  ...
  {
    id: "mentorcomercial",
    name: "MentorComercial (Execally)",
    links: {
      prod: "https://execally.online",
      dev: "http://localhost:3000",
      github: "https://github.com/saadypacheco/mentorcomercial",
      docs: "https://github.com/saadypacheco/AsitenteComercial",
    },
    credentials: [
      { id: "1", label: "Login demo — email", value: "saadypacheco@gmail.com", sensitive: false },
      { id: "2", label: "Login demo — password", value: "Cecilia2026", sensitive: true },
      { id: "3", label: "API URL", value: "https://api.execally.online", sensitive: false },
      { id: "4", label: "GitHub branch activo", value: "001-captura-whatsapp-bd", sensitive: false },
      { id: "5", label: "Supabase URL", value: "", sensitive: false },
      { id: "6", label: "Supabase Anon Key", value: "", sensitive: true },
      { id: "7", label: "RESEND_API_KEY (emails reales)", value: "", sensitive: true },
      { id: "8", label: "GEMINI_API_KEY (IA real)", value: "", sensitive: true },
      { id: "9", label: "TELEGRAM_BOT_TOKEN", value: "", sensitive: true },
      { id: "10", label: "WAHA API URL", value: "", sensitive: false },
      { id: "11", label: "WAHA API Key", value: "", sensitive: true },
    ],
    infra: {
      docker: "/docker/mentorcomercial/infra/docker-compose.prod.yml",
      supabase: "Supabase Cloud (base de datos principal)",
      other: "VPS Hostinger Ubuntu · IP 76.13.234.191 · Traefik HTTPS automático · 4 contenedores",
    },
    comms: { slack: "#mentor-comercial" },
    notes: "CLIENTE: Cecilia — agencia de seguros con ~2.000 agentes.\n\nQUÉ HACE: Captura automática de grupos WhatsApp → ordena en dashboard mobile-first → IA detecta oportunidades, clientes en riesgo, agentes estancados → sugiere y ejecuta acciones → briefing diario → asistente conversacional. Onboarding de agentes y paneles segmentados por líder.\n\nCONTENEDORES (4): mc-frontend (3000) + mc-backend (8002) + mc-worker (8002) + mc-waha (WAHA bridge, 402MB RAM).\n\nVOLÚMENES: mentorcomercial_waha_sessions + mentorcomercial_waha_media.\n\nESTADO HOY: Captura WhatsApp conectada a número observador REAL. Datos = mezcla demo sembrada + mensajes reales capturados.\n\nCLAVES OPCIONALES (el sitio funciona sin ellas con fallbacks): RESEND_API_KEY → emails reales. GEMINI_API_KEY → IA real en vez de determinista. TELEGRAM_BOT_TOKEN → asistente por Telegram. Se cargan en backend/.env del VPS.\n\nREPO: privado (código completo) en mentorcomercial branch 001-captura-whatsapp-bd. Mirror público curado (puede estar atrasado): AsitenteComercial.",
    updatedAt: "",
  },
  {
